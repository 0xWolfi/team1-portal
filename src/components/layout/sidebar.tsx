  ...
  const NavContent = () => (
    <>
      ...
      <div className="h-16 flex items-center gap-3 px-5 border-b border-white/[0.06] shrink-0">
        <img src="/logos/Team1_Symbol_Main.svg" alt="team1" className="h-6 w-6" />
        <span className="text-white font-medium">team1 <span className="text-brand-500">portal</span></span>
        {isAdmin && (
          <span className="ml-auto text-[10px] font-mono text-brand-400 bg-brand-500/10 px-2 py-0.5 rounded-full border border-brand-500/20">
            admin
          </span>
        )}
        {!isAdmin && (
      ...
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {active && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.4)]" />}
            </Link>
          )
        })}
        ...
        {isAdmin && regions.length > 0 && (
          <div className="pt-4 mt-4 border-t border-white/[0.06]">
            <p className="px-3 mb-2 text-[11px] font-medium text-zinc-600 uppercase tracking-wider">{isSuperAdmin ? 'Regions' : 'Your Regions'}</p>
                ...
                <div key={r.slug}>
                  <button
                    ...
                    className={cn(
                      ...
                      expanded ? 'text-white bg-white/[0.06]' : 'text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.04]'
                    )}
                  >
                      ...
                      {[
                        ...
                        <Link
                          ...
                          className={cn(
                            ...
                            pathname === sub.href ? 'text-white bg-white/[0.06]' : 'text-zinc-600 hover:text-zinc-300'
                          )}
                        >
                          {sub.label}
                        </Link>
        ...
        {!isAdmin && (isSuperAdmin || isRegionLead()) && (
          <div className="pt-4 mt-4 border-t border-white/[0.06]">
            <Link
              href="/admin"
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-zinc-500 hover:text-brand-400 hover:bg-brand-500/[0.06] transition-all"
            >
        ...
        {isAdmin && (
          <div className="pt-4 mt-4 border-t border-white/[0.06]">
            <Link
              href="/dashboard"
              className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.04] transition-all"
            >
      ...
      <div className="px-3 py-3 border-t border-white/[0.06] space-y-0.5 shrink-0">
        <Link
          ...
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group text-zinc-500 hover:text-zinc-200 hover:bg-white/[0.04]"
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium truncate text-zinc-300">{user?.displayName || 'Profile'}</p>
            <p className="text-[11px] text-zinc-600 truncate">{user?.email}</p>
          </div>
        </Link>
        <button
          onClick={handleLogout}
          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13px] font-medium text-zinc-600 hover:text-red-400 hover:bg-red-500/[0.06] transition-all duration-200 cursor-pointer"
        >
          <LogOut size={18} />
          Sign out
        </button>
    ...
    <>
      ...
      <header className="lg:hidden sticky top-0 z-50 px-4 py-3 bg-black/40 backdrop-blur-2xl border-b border-white/[0.06] flex items-center justify-between">
        <div className="flex items-center gap-2">
          <button onClick={() => setMobileOpen(true)} className="p-2 -ml-2 rounded-lg text-zinc-400 hover:text-white hover:bg-white/5 transition-colors">
            <Menu size={20} />
          </button>
          <img src="/logos/Team1_Symbol_Main.svg" alt="team1" className="h-5 w-5" />
          <span className="text-white text-sm font-medium">team1</span>
        </div>
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 z-[60]">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-[280px] flex flex-col bg-zinc-950/95 backdrop-blur-2xl border-r border-white/[0.06]">
            <NavContent />
          </aside>
        </div>
      )}
