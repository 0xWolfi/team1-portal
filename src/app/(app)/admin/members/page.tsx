...
export default function AdminMembersPage() {
  ...
  const handleAddMember = async () => {
    if (!addForm.email || !addForm.regionId) {
      showError('Email and region are required')
      return
    }
    const res = await mutate('/api/admin/members', 'POST', addForm)
    if (res.success) {
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-medium text-white flex items-center gap-2"><Users size={24} className="text-red-500" /> All Members</h1>
          <p className="text-sm text-zinc-400 mt-1">{result?.total || 0} total members</p>
        </div>
      ...
      <div className="flex gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[200px] max-w-md">
          ...
          <input className="w-full h-10 pl-9 pr-4 bg-zinc-900/50 border border-white/5 rounded-lg text-sm text-zinc-300 placeholder:text-zinc-500 focus:outline-none focus:border-red-500" placeholder="Search by name, email..." value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <select className="h-10 px-3 bg-zinc-900/50 border border-white/5 rounded-lg text-sm text-zinc-300 focus:outline-none focus:border-red-500" value={regionFilter} onChange={(e) => setRegionFilter(e.target.value)}>
      ...
      <Modal open={addModal} onClose={() => setAddModal(false)} title="Add Member">
        <div className="space-y-4">
          <Input
            ...
            className="bg-zinc-900/50 border-white/5 rounded-xl"
          />
          <div className="space-y-1.5">
            <label className="block text-sm text-zinc-300 font-medium">Region</label>
            <select
              className="w-full h-11 px-4 bg-zinc-900/50 border border-white/5 rounded-xl text-sm text-zinc-200 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/30"
          ...
          <div className="space-y-1.5">
            <label className="block text-sm text-zinc-300 font-medium">Role</label>
            <select
              className="w-full h-11 px-4 bg-zinc-900/50 border border-white/5 rounded-xl text-sm text-zinc-200 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500/30"
              value={addForm.role}
        ...
        {selectedMember && (
          <div className="fixed inset-0 z-[70] flex justify-end">
            ...
            <motion.div
              ...
              className="relative w-full max-w-md bg-zinc-900/50 border-l border-white/5 shadow-2xl overflow-y-auto"
            >
              <div className="flex items-center justify-between px-6 py-4 border-b border-white/5 sticky top-0 bg-zinc-900 z-10">
                ...
                <button onClick={() => setSelectedMember(null)} className="text-zinc-400 hover:text-white transition-colors cursor-pointer">
                  <X size={20} />
                </button>
              </div>
              <div className="p-6 space-y-6">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="text-lg font-medium text-white">{selectedMember.user.displayName}</h3>
                    {selectedMember.user.username && (
                      <p className="text-sm text-zinc-400">@{selectedMember.user.username}</p>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-zinc-900/50 border border-white/5 rounded-2xl p-4 space-y-3">
                  ...
                  {selectedMember.user.createdAt && (
                    <div className="flex justify-between">
                      ...
                      <span className="text-sm text-zinc-300">{formatDate(selectedMember.user.createdAt)}</span>
                    </div>
                  )}
                </div>
