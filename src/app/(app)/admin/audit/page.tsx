...
const MODULES = ['', 'profile', 'members', 'roles', 'activities', 'roster', 'leads', 'regions', 'announcements', 'applications', 'playbooks']
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-3 flex-wrap">
        <select
          ...
          className="h-10 px-3 bg-zinc-900/50 border border-white/5 rounded-lg text-sm text-zinc-300 focus:outline-none focus:border-red-500"
        >
        ...
        <input
          ...
          className="h-10 px-3 bg-zinc-900/50 border border-white/5 rounded-lg text-sm text-zinc-300 placeholder:text-zinc-500 focus:outline-none focus:border-red-500 min-w-[260px]"
        />
      </div>
        ...
        <Card className="overflow-hidden p-0">
          <table className="w-full">
            <thead><tr className="border-b border-zinc-700/50">
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-zinc-400 uppercase">Entity</th>
              ...
              {result.items.map((log) => {
                ...
                return (
                  <>
                    <tr
                      key={log.id}
                      className={`border-b border-zinc-700/50 hover:bg-zinc-800/50 ${hasDiff ? 'cursor-pointer' : ''}`}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-white font-mono">{log.action}</td>
                    ...
                    <AnimatePresence>
                      {expanded && hasDiff && (
                        <tr key={`${log.id}-diff`} className="border-b border-zinc-700/50 bg-zinc-950/40">
