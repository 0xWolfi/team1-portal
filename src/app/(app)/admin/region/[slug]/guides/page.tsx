  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between">
        <div><h1 className="text-2xl font-medium text-white flex items-center gap-2"><FileText size={24} className="text-red-500" /> Guides</h1>
        <p className="text-sm text-zinc-400 mt-1">Manage guides for {slug}</p></div>
        ...
        <div className="space-y-3">{guides.map((g) => (
          <Card key={g.id} className="flex items-center justify-between">
            <div className="flex-1"><div className="flex items-center gap-2 mb-1">
              <h3 className="text-sm font-medium text-white">{g.title}</h3>
            ...
            <div className="flex gap-1">
              ...
              <Button variant="ghost" size="sm" onClick={() => handleDelete(g.id)}><Trash2 size={14} className="text-red-400" /></Button>
            </div>
          </Card>
        ))}</div>
